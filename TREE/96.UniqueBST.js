...
* root-1 n-R   root-1  n-R     root-1  n-R    root-1 node-root  *
*/
var numTrees = function(n) {
  ...
  for (let node=2; node<=n; node++) { //for this many given nodes
    for (let root=1; root<=node; root++) { //try dif roots starting from 1
      dp[node] += dp[root-1] * dp[node-root]; //root-1 is roots lesser than cur root. node-root is roots greater than cur root
    }
  }
  return dp[n];
};
